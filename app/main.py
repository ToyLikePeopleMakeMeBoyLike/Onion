            ...
            if existing_entry:
                ...
                if custom_alias:
                    if not (3 <= len(custom_alias) <= 30 and custom_alias.isalnum()): # Basic validation for alias
                        ...
                        custom_alias = None # Invalidate custom alias
                    else:
                ...
                if not custom_alias and not existing_entry: # Only generate if no custom alias and not already existing
                    while True:
                        short_code = generate_short_code()
                        existing_code = query_db('SELECT id FROM urls WHERE short_code =?', [short_code], one=True)
                        if not existing_code:
                            break
                
                if 'short_code' in locals() and short_code: # Ensure short_code was set
                    try:
                        execute_db('INSERT INTO urls (original_url, short_code) VALUES (?,?)',
                                   [original_url, short_code])
                        current_app.logger.info(f"Stored new mapping: {short_code} -> {original_url}")
                        short_url_display = request.host_url.rstrip('/') + url_for('main.redirect_to_url', short_code=short_code)
                        flash(f"URL shortened successfully!", "success")
                    except sqlite3.IntegrityError:
                        flash(f"Could not create short URL due to a conflict (alias might have been taken). Please try again.", "error")
                        current_app.logger.error(f"Database integrity error for short_code {short_code}")
                elif not existing_entry and not custom_alias: # If we failed to set a short_code and it wasn't an existing entry
